<div align="center">
  <img src="https://raw.githubusercontent.com/Vall-Here/Simple-Tools/main/assets/logo.png" width="150" alt="Simple Tools Logo">
  <h1>🛠️ Simple Tools</h1>
  <p>
    <b>The Ultimate, All-In-One Offline Document & Image Converter for Android</b>
  </p>
  
  [![Version](https://img.shields.io/badge/Version-1.4.0-blue.svg)](https://github.com/Vall-Here/Simple-Tools/releases)
  [![Platform](https://img.shields.io/badge/Platform-Android-green.svg)](https://developer.android.com)
  [![License](https://img.shields.io/badge/License-MIT-purple.svg)](LICENSE)
  
  <br/>
  <br/>
  
  <a href="https://github.com/Vall-Here/Simple-Tools/raw/main/app/Simple-Tools-v1.4.0.apk">
    <img src="https://img.shields.io/badge/Download-APK%20v1.4.0-green?style=for-the-badge&logo=android" alt="Download APK">
</a>
  </a>
</div>

---

## 🌟 Why Simple Tools?

Converting files shouldn't require an internet connection, a subscription, or waiting in queues. **Simple Tools** does all the heavy lifting directly on your device. Zero servers, zero tracking, 100% privacy!

## 🆕 What's New in v1.4.0

- **Multi-Stamp PDF Signature:** Support adding multiple signatures anywhere on a single PDF page easily with drag and drop preview!
- **Improved Watermarks:** Fixed PDF text/image watermark crash and optimized watermark size and transparency parameters.
- **EXIF Stripper:** Securely strip hidden metadata (GPS, camera make/model, timestamp) from your images to protect your privacy!
- **Dynamic Output Folder:** All processed files are now automatically saved to `Downloads/SimpleTools`, with the flexibility to customize the output folder from the settings.

### ✨ Core Features

- **Compress Image:** Reduce photo size (JPG/PNG) offline.
- **Merge PDF:** Combine multiple PDF files into a single document easily.
- **Image to PDF:** Convert images (JPG/PNG) into a single PDF file.
- **Split PDF:** Extract specific pages from a PDF into separate documents.
- **Compress PDF:** Smart compression to drastically reduce PDF file size.
- **Word to PDF:** Convert DOCX documents to PDF seamlessly.
- **PDF to Word:** Extract text from PDFs into editable Word (DOCX) documents.
- **Scanner:** Premium document scanner with auto-edge detection and B&W filters.
- **QR & Barcode:** Scan codes securely without camera permissions, or generate custom QR (with logos & colors) and Barcodes offline.
- **EXIF Stripper:** Remove hidden metadata (GPS, Camera info) from your photos to protect your privacy before sharing.
- **History:** Easy access to all your previously converted files and processes.

### 🚀 Built for Productivity

- **Batch Processing**: Convert multiple files at once and reorder them easily.
- **Offline First**: Works anywhere, anytime. No internet required.
- **Smart History**: All your processed files are safely stored in your `Downloads/SimpleTools` folder and easily accessible directly from the app's _History_ screen!

---

## 📸 Screenshots

![Screenshoot](assets/sc.jpeg)

## 📥 Installation

1. Download the latest APK by clicking the **Download APK** button above.
2. Locate the downloaded `Simple-Tools-v1.4.0.apk` file on your Android device.
3. Tap to install (make sure to allow installation from unknown sources if prompted).
4. Enjoy converting entirely offline!

## 🛠️ Technical Stack

- **UI Framework:** Jetpack Compose (Material 3)
- **Architecture:** MVVM, Coroutines, StateFlow
- **PDF Engine:** Apache PDFBox (Android Port)
- **Document Engine:** Apache POI

## 🤝 Contributing

Contributions, issues, and feature requests are welcome! Contact Me (ahmadnoval.muhyiddin@gmail.com).
